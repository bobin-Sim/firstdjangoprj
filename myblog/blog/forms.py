...
# 모델에서 Model.models를 괄호 안에 넣었듯이
# 폼 생성에서도 class 폼이름(forms.ModelForm): 으로 시작해야함
class PostForm(forms.ModelForm):
    
    # 폼의 상세정보 설정을 위해 class PostForm 내부에
    # class Meta를 다시 만들어야 함. Meta에 정의한 정보대로
    # 폼이 생성 됨.
    
    class Meta:
        # 이 폼의 타겟이 Post 모델(Post 모델에 적재 예정)
        model = Post
        # 실제로 사용자에게 입력 받을 컬럼은 title과 text만
        # author은 자동으로 계정 연동, create_date는 서버시간
        # published_date는 퍼블리싱 할 때 추후 입력
